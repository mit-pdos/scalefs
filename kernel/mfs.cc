...
s64
readm(sref<mnode> m, char* buf, u64 start, u64 nbytes)
{
...
s64
writem(sref<mnode> m, const char* buf, u64 start, u64 nbytes,
       mfile::resizer* parentresize)
{
  ...
  while (start + off < end) {
    ...
    if (pi) {
      ...
      m->as_file()->dirty(true);
      m->as_file()->set_page_dirty(pgbase / PGSIZE);
