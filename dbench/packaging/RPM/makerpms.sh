...
# (copied from ctdb, which copied it from samba)
#
...
SPECFILE="dbench.spec"
SPECFILE_IN="dbench.spec.in"
RPMBUILD="rpmbuild"
...
pushd ${TOPDIR}
echo -n "Creating dbench-${VERSION}.tar.gz ... "
git archive --prefix=dbench-${VERSION}/ HEAD | ${GZIP} > ${SRCDIR}/dbench-${VERSION}.tar.gz
RC=$?
popd
echo "Done."
if [ $RC -ne 0 ]; then
